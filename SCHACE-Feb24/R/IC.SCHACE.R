...
IC.SCHACE <- function(A, N, sample_x, sample_y, Bdf_set, IC){
  ...
  for (j in 1:length(Bdf_set)){
    ...
    bsp_mat = bs(sample_x, df = Bdf_set[j])

    X = cbind(A, bsp_mat[, 1:Bdf_set[j]])

    object = glmnet(X, sample_y, family = "gaussian", penalty.factor = c(rep(1, N-1), rep(0, Bdf_set[j])),
